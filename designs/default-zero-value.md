...
- Structure: an empty structure. Structures only have a default zero value if
  all members of the structure are optional (that is, no member is marked with
  the `@required` trait).
...
- When the `@default` trait is present, a corresponding accessor SHOULD always
  return a non-null value by defaulting missing members with their zero values.
- When the `@required` trait is present, a corresponding accessor SHOULD always
...
- Because the `@required` trait can be backward-compatibly removed from members
  ...
  operation), code generators SHOULD generate code that does not break if the
...
Some of this guidance will admittedly be an inconvenience for developers, but
it is less catastrophic than previously generated client code failing at
runtime when deserializing the response of an operation, and it matches the
current reality of how AWS was modeled. While it's true client code might
start failing anyways if a service stops sending output members that were
...
SDK fails during deserialization because a required output member is missing,
then customer applications would be completely broken until they update their
SDK.


#### Special casing top-level input structures

AWS SDKs SHOULD generate every required top-level input member as optional even
if they're marked as required (that is, the members of structures marked with
the `@input` trait). This gives AWS service teams the ability to remove the
required trait from top-level input members without risking breaking previously
generated clients. If such a change would not break previously generated
clients for a given AWS SDK, then it is free to ignore this guidance.

Nearly 80% of all required traits used in AWS today appear in only top-level
members of operation inputs (~24,500 total required members vs ~19,000
required top-level input members). By special casing top-level operation input
members as optional even if they are marked with the required trait, this
proposal becomes significantly safer for AWS SDKs to adopt.


#### `aws.api#serverRequired` trait

The `aws.api#serverRequired` trait is used instead of `required` to indicate
that a structure member is required by the server, but MUST NOT be used by
clients to influence generated types. This trait allows documentation
generators to indicate that a member is required, even if it is not reflected
in generated code.

The `aws.api#serverRequired` trait is defined in Smithy as:

```
$version: "2"
namespace aws.api

@trait(selector: "structure > member", conflicts: [required, default])
structure serverRequired {}
```
...
##### Backfilling the serverRequired trait

The `aws.api#serverRequired` trait will be backfilled onto AWS models as
needed. Some services will replace every required structure member with this
trait, whereas others will only use it on structure members that target
structure or union shapes that have no default zero value. Over time, as models
are audited and corrected, we can release improved AWS SDKs with better support
for the required trait.
...
### Why special-case backward compatibility rules for top-level inputs?

Services evolve over time. What was required as input 10 years ago might not
need to be required in the future as new features are added to a service.
Removing the required trait from a top-level operation input member is relaxing
a requirement, which makes it a backward compatible change. Smithy code
generators SHOULD take this into account to avoid breaking previously generated
code if the required trait is removed from a top-level operation input member.


### What is the `@input` and `@output` trait?

This will be covered in a separate design doc, but at a high level, Smithy 2.0
will require that all operations have input and output, the input shape MUST
be marked with the `@input` trait, the output shape MUST be marked with the
`@output` trait, and these shapes can only be referenced from a single
operation. This change simplifies the metamodel, it removes complicated
logic previously needed in AWS SDKs to "synthesize" input and output shapes for
operations, and it makes modelers think more deliberately about how an
